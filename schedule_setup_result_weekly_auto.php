...
function breakCodeInsertion($total_user,$per_slot_user,$shift,$breakValue,$colum,$columData, $start ){
    ...
    if($breakValue!="lunch_break"){
        ...
        if ($TYPE == "SET_ROSTER") {
            ...
            $agentArray=array($_POST['dayWiseAgent0'],$_POST['dayWiseAgent1'],$_POST['dayWiseAgent2'],$_POST['dayWiseAgent3'],$_POST['dayWiseAgent4'],$_POST['dayWiseAgent5'],$_POST['dayWiseAgent6']);
         ...
         foreach ($dateArr as $startDate) {
            ...
            $userInRosterStr='';
            $countUser=mysql_num_rows($selectUser);
            ...
            if ($countUser==0) 
            {
                $user = mysql_query("SELECT * FROM `roster`.`users_info` WHERE `schedule_date`='$startDate' AND `active_status`=1 AND `user_type`='$userType' AND `campaign_name`='$campName' LIMIT $agentArray[$check]");
                // echo "SELECT * FROM `roster`.`users_info` WHERE `schedule_date`='$startDate' AND `active_status`=1 AND `user_type`='$userType' AND `campaign_name`='$campName' LIMIT $agentArray[$check]";
            }
            else
            {
                $user = mysql_query("SELECT * FROM `roster`.`users_info` WHERE `schedule_date`='$startDate' AND `active_status`=1 AND `user_type`='$userType' AND `campaign_name`='$campName' AND `user_name` NOT IN (" .$userInRosterStr. ") LIMIT $agentArray[$check]");
                // echo "SELECT * FROM `roster`.`users_info` WHERE `schedule_date`='$startDate' AND `active_status`=1 AND `user_type`='$userType' AND `campaign_name`='$campName' AND `user_name` NOT IN (" .$userInRosterStr. ") LIMIT $agentArray[$check]";
            }
