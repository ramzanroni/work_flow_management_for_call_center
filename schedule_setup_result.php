...
function breakCodeInsertion($total_user,$per_slot_user,$shift,$breakValue,$colum,$columData, $start ){
    ...
    if($breakValue!="lunch_break"){
        ...
        if ($TYPE == "SET_ROSTER") {
            ...
            // $check_sql = mysql_num_rows(mysql_query("SELECT * FROM `db_shooz`.`fifteen_minute_schedul` WHERE `create_date` = '".$start."' AND `shift_name`='$shift'"));

            // if ($check_sql == 0) {
                $user = mysql_query("SELECT * FROM `roster`.`users_info` WHERE `schedule_date`='$start' AND `active_status`=1 AND `user_type`='$userType' AND `campaign_name`='$campName' AND `id` IN (" .$userIds. ")");
        ...
        // workstatus
            ...
            //echo $breakValue;
                ...
                // } else {
                //    echo "This data Already In Database!";
                // }
            } else{
                echo "not working";
            }
